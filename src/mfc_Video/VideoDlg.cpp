...
CVideoDlg::CVideoDlg(CWnd* pParent /*=nullptr*/) : CDialogEx(IDD_VIDEO_DIALOG, pParent)
{
...
void CVideoDlg::OnBnClickedOpenImg()
{
    ...
    CFileDialog fdlg(TRUE,
                     ...
                     L"图片文件(*.jpg;*.png;*.bmp;*.gif)|*.jpg;*.png;*.bmp;*.gif|"
                     L"JPEG文件(*.jpg)|*.jpg|"
                     L"PNG文件(*.png)|*.png|"
                     L"所有文件(*.*)|*.*||",
                     this);
    ...
    /// 设置对话框标题
    fdlg.m_ofn.lpstrTitle      = L"选择图片文件";
    ...
    /// 打开图像
    cv::Mat mat = cv::imread(W2A(fdlg.GetPathName().GetBuffer()));
    if (mat.empty())
    {
        MessageBox(L"img open failed!");
        return;
